---
name: troubleshoot-env-checker
description: Use when assessing or diagnosing a support ticket involving Qubership Environment Checker (Environment Checker, env-checker) or its components (RBAC and cluster access, installation, S3 report storage, monitoring metrics, Git integration, UI authentication, notebook execution), including installation, configuration, and runtime failures. Read-only and advisory; no live system access.
---
...
Diagnose one reported problem with Qubership Environment Checker. Read the user's description and any attached evidence,
...
2. **Localize.** From the text and the log signatures, name the **component** (RBAC and cluster access, installation,
   S3 report storage, monitoring metrics, Git integration, UI authentication, notebook execution).
